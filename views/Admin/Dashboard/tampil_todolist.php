	...
	if ($result->num_rows > 0) {
		if ($result->num_rows <= 10) {
        	while ($row = $result->fetch_assoc()) {
				...
				echo '</div>';
				echo "<button type='button' id='".$row['id']."' class='btn btn-outline-secondary btn-rounded btn-icon remove' style='float:right;'><i class='mdi mdi-close-circle-outline text-primary'></i></button>";
				echo '</li>';
			}
		}
	}else{
